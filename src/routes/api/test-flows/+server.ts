...
export async function GET({ locals }: RequestEvent) {
  try {
    ...
    // Get all test flows for the authenticated user
    const userTestFlows = await db.select({
      id: testFlows.id,
      name: testFlows.name,
    ...
    const testFlowApiAssociations = await db.select({
    ...
    const testFlowApisMap = testFlowApiAssociations.reduce((acc, item) => {
...
export async function POST({ request, locals }: RequestEvent) {
  try {
    ...
    const userApis = await db.select({ id: apis.id })
      .from(apis)
      .where(and(
    ...
    if (userApis.length !== apiIds.length) {
      return new Response(JSON.stringify({ error: 'One or more APIs not found or do not belong to the user' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    const [newTestFlow] = await db.insert(testFlows)
      .values({
        name,
        description,
...
export async function DELETE({ request, locals }: RequestEvent) {
  try {
    ...
    const [existingTestFlow] = await db.select()
      .from(testFlows)
      .where(and(
        eq(testFlows.id, id),
        eq(testFlows.userId, locals.user.userId)
      ));

    if (!existingTestFlow) {
      return new Response(JSON.stringify({ error: 'Test flow not found or does not belong to the user' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json' }
      });
    }
