...
describe('ExpressionEvaluator Map Function - User Guide & Test Suite', () => {
  ...
  describe('Object Transformation Mapping', () => {
    ...
    it('should map to create objects with renamed fields', () => {
      const result = evaluator.evaluate(
        '$.products | map(productId: $.id, title: $.name, cost: $.price)',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('should map to create objects with nested field flattening', () => {
      const result = evaluator.evaluate(
        '$.users | map(userId: $.id, userName: $.name, city: $.profile.city, country: $.profile.country)',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Template Expression Support', () => {
    ...
    it('should support parameter templates in object mapping', () => {
      const result = evaluator.evaluate(
        '$.users | map(user: "{{param:username}}", name: $.name, role: "{{param:userRole}}")',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('should support parameter templates for environment-mapped values', () => {
      const result = evaluator.evaluate(
        '$.products | map(app: "{{param:appName}}", product: $.name, currency: "{{param:defaultCurrency}}")',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Complex Pipeline Expressions in Map - User Examples', () => {
    ...
    it('should support template expressions like the user requested', () => {
      ...
      const result = evaluator.evaluate(
        '$.users | map(username: "{{param:username}}", balance: $.balance)',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('should support numeric template calculations', () => {
      const result = evaluator.evaluate(
        '$.users | map(id: $.id, boostedBalance: $.balance)',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Advanced Use Cases', () => {
    it('should map with mixed data types', () => {
      const result = evaluator.evaluate(
        '$.users | map(id: $.id, name: $.name, isActive: $.active, balance: $.balance)',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('should support mapping with array operations', () => {
      const result = evaluator.evaluate(
        '$.orders | map(order: $.orderId, itemCount: $.items, total: $.total)',
        sampleData
      );
      expect(result).toEqual([
        {
    ...
    it('should support complex nested object creation', () => {
      ...
      const result = evaluator.evaluate(
        '$.users | map(profile: $.id, displayName: $.name, location: $.profile.city, tier: "{{param:userRole}}")',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should throw for template expressions with missing parameters', () => {
      expect(() =>
        evaluator.evaluate(
          '$.users | map(name: $.name, param: "{{param:nonexistentParam}}")',
          sampleData
        )
    ...
    it('should handle objects with computed fields', () => {
      const result = evaluator.evaluate(
        '$.users | map(id: $.id, name: $.name, role: "{{param:userRole}}")',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Performance and Chaining', () => {
    it('should work efficiently with other pipeline functions', () => {
      const result = evaluator.evaluate(
        '$.users | where($.active == true) | map($.name) | sort()',
        sampleData
      );
    ...
    it('should support chaining with where clauses', () => {
      const result = evaluator.evaluate(
        '$.users | where($.balance > {{param:baseAmount}}) | map(user: $.name, balance: $.balance)',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Real-world Scenarios', () => {
    it('should transform API response for frontend consumption', () => {
      const result = evaluator.evaluate(
        '$.users | map(userId: $.id, displayName: $.name, location: $.profile.city, currency: "{{param:defaultCurrency}}")',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('should create summary reports with template integration', () => {
      const result = evaluator.evaluate(
        '$.products | map(product: $.name, price: $.price, category: $.category, app: "{{param:appName}}")',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('should support data normalization with response templates', () => {
      const result = evaluator.evaluate(
        '$.users | map(id: $.id, prefix: "{{res:step1-0.$.userPrefix}}", name: $.name)',
        sampleData
      );
      expect(result).toEqual([
  ...
  describe('Template Expression Use Cases Guide', () => {
    it('demonstrates {{param:name}} usage in map function', () => {
      ...
      const result = evaluator.evaluate(
        '$.users | map(role: "{{param:userRole}}", user: $.name)',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('demonstrates parameter usage for environment-mapped values in map function', () => {
      const result = evaluator.evaluate(
        '$.products | map(name: $.name, app: "{{param:appName}}", currency: "{{param:defaultCurrency}}")',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('demonstrates {{res:step.path}} usage in map function', () => {
      ...
      const result = evaluator.evaluate(
        '$.users | map(id: $.id, prefix: "{{res:step1-0.$.userPrefix}}", multiplier: {{res:step1-0.$.multiplier}})',
        sampleData
      );
      expect(result).toEqual([
    ...
    it('demonstrates complex template combinations', () => {
      ...
      const result = evaluator.evaluate(
        '$.users | map(user: "{{param:username}}", app: "{{param:appName}}", id: $.id, prefix: "{{res:step1-0.$.userPrefix}}")',
        sampleData
      );
      expect(result).toEqual([
