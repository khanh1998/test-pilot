...
describe('Arithmetic Pipeline Functions', () => {
  ...
  describe('add() function', () => {
    ...
    it('should work with parameter-mapped environment values', () => {
      const result = evaluator.evaluate('{{param:baseAmount}} | add({{param:serviceFee}})', {});
      expect(result).toBe(103.5); // 100 + 3.5
    });

    it('should work with transformed data templates', () => {
      const result = evaluator.evaluate(
        '{{proc:step1-0.$.calculated.subtotal}} | add({{proc:step1-0.$.calculated.shipping}})',
        {}
      );
  ...
  describe('sub() function', () => {
    ...
    it('should work with template expressions', () => {
      const result = evaluator.evaluate(
        '{{res:step1-0.$.total}} | sub({{res:step1-0.$.discount}})',
        {}
      );
  ...
  describe('mul() function', () => {
    ...
    it('should work with template expressions', () => {
      const result = evaluator.evaluate(
        '{{res:step1-0.$.price}} | mul({{res:step1-0.$.quantity}})',
        {}
      );
    ...
    it('should work with parameter-mapped environment values', () => {
      const result = evaluator.evaluate('{{param:baseAmount}} | mul({{param:exchangeRate}})', {});
  ...
  describe('div() function', () => {
    ...
    it('should work with template expressions', () => {
      const result = evaluator.evaluate(
        '{{res:step1-0.$.total}} | div({{res:step2-0.$.divisor}})',
        {}
      );
  ...
  describe('mod() function', () => {
    ...
    it('should work with template expressions', () => {
      const result = evaluator.evaluate(
        '{{res:step1-0.$.user_id}} | mod({{res:step2-0.$.mod_value}})',
        {}
      );
  ...
  describe('Chained arithmetic operations', () => {
    ...
    it('should chain with template expressions', () => {
      const result = evaluator.evaluate(
        '{{param:basePrice}} | mul({{param:quantity}}) | add({{param:serviceFee}}) | sub({{res:step1-0.$.discount}})',
        {}
      );
    ...
    it('should handle complex calculation pipeline', () => {
      ...
      const result = evaluator.evaluate(
        '{{res:step1-0.$.price}} | mul({{res:step1-0.$.quantity}}) | add({{proc:step1-0.$.calculated.shipping}}) | sub({{res:step1-0.$.discount}})',
        {}
      );
  ...
  describe('Integration with other pipeline functions', () => {
    it('should work with sum and arithmetic', () => {
      ...
      const result = evaluator.evaluate(
        '$.items | sum($.price) | add({{param:serviceFee}}) | mul({{param:exchangeRate}})',
        data
      );
    ...
    it('should work with count and arithmetic', () => {
      ...
      const result = evaluator.evaluate(
        '$.items | count() | mul({{param:basePrice}}) | div({{param:quantity}})',
        data
      );
    ...
    it('should work with at() and arithmetic', () => {
      const data = {
        prices: [10.0, 15.5, 20.75, 30.25]
      };

      const result = evaluator.evaluate(
        '$.prices | at(2) | mul({{param:factor}}) | add({{param:serviceFee}})',
        data
      );
  ...
  describe('Real-world use cases with templates', () => {
    it('should calculate order total with tax', () => {
      ...
      const expression =
        '{{res:step1-0.$.price}} | mul({{res:step1-0.$.quantity}}) | mul({{res:step1-0.$.tax_rate}} | add(1)) | add({{proc:step1-0.$.calculated.shipping}}) | sub({{res:step1-0.$.discount}})';
    ...
    it('should calculate compound interest', () => {
      ...
      const result = evaluator.evaluate(
        '{{param:baseAmount}} | mul({{res:step1-0.$.tax_rate}} | add(1))',
        {}
      );
    ...
    it('should calculate currency conversion with fees', () => {
      ...
      const result = evaluator.evaluate(
        '{{res:step1-0.$.total}} | mul({{param:exchangeRate}}) | add({{param:serviceFee}})',
        {}
      );
    ...
    it('should calculate shipping cost based on weight tiers', () => {
      ...
      const result = evaluator.evaluate(
        '{{proc:step1-0.$.calculated.shipping}} | add({{proc:step1-0.$.calculated.fees}} | mul({{res:step2-0.$.multiplier}}))',
        {}
      );
    ...
    it('should calculate bulk discount tiers', () => {
      ...
      const result = evaluator.evaluate(
        '{{res:step1-0.$.quantity}} | div(10) | mul({{res:step1-0.$.price}}) | sub({{res:step1-0.$.discount}})',
        {}
      );
  ...
  describe('Error handling and edge cases', () => {
    it('should throw clear errors for missing template values', () => {
      expect(() => evaluator.evaluate('10 | add({{res:nonexistent.$.value}})', {})).toThrow(
  ...
  describe('Template evaluation contexts', () => {
    it('should evaluate templates in arithmetic context correctly', () => {
      ...
      const result = evaluator.evaluate(
        '{{res:step1-0.$.user_id}} | add({{res:step2-0.$.bonus}}) | mul({{param:factor}})',
        {}
      );
    ...
    it('should handle nested template references', () => {
      ...
      const result = evaluator.evaluate(
        '{{proc:step1-0.$.calculated.subtotal}} | add({{res:step1-0.$.user_id}} | div(10))',
        {}
      );
    ...
    it('should maintain template context through pipeline', () => {
      ...
      const expression =
        '{{param:basePrice}} | mul({{param:quantity}}) | add({{param:serviceFee}}) | mul({{param:exchangeRate}}) | sub({{res:step1-0.$.discount}}) | div({{res:step2-0.$.divisor}})';
      const result = evaluator.evaluate(expression, {});
