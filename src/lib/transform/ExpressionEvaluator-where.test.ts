...
describe('ExpressionEvaluator Where Function with Comparison Expressions', () => {
  ...
  it('should filter using equality comparison', () => {
    ...
    const expected = [{ name: 'Alice', age: 25, active: true }];
  ...
  it('should filter using string equality comparison', () => {
    ...
    const expected = [{ name: 'Bob', age: 17, active: false }];
...
describe('ExpressionEvaluator Where Function with Template Expressions', () => {
  ...
  it('should support template expressions with numeric parameters - the exact example', () => {
    ...
    const result = evaluator.evaluate(
      '$.data | where($.balance > {{param:minBalance}}) | map($.username)',
      testData
    );
    const expected = ['alice', 'charlie', 'eve'];
  ...
  it('should support template expressions with string parameters', () => {
    const result = evaluator.evaluate(
      '$.data | where($.status == "{{param:targetStatus}}")',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
  ...
  it('should support unquoted template expressions preserving numeric types', () => {
    const result = evaluator.evaluate(
      '$.data | where($.balance <= {{param:maxBalance}})',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
  ...
  it('should support parameter-mapped environment values in template expressions', () => {
    const result = evaluator.evaluate(
      '$.data | where($.category == "{{param:filterCategory}}")',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
  ...
  it('should support numeric parameter-mapped environment values', () => {
    const result = evaluator.evaluate(
      '$.data | where($.balance >= {{param:minThreshold}})',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
  ...
  it('should support complex expressions with multiple template parameters', () => {
    const result = evaluator.evaluate(
      '$.data | where($.balance > {{param:minBalance}} && $.status == "{{param:targetStatus}}")',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
  ...
  it('should support template expressions in complex pipelines', () => {
    const result = evaluator.evaluate(
      '$.data | where($.balance > {{param:minBalance}}) | where($.category == "{{param:filterCategory}}") | map($.username)',
      testData
    );
    const expected = ['alice', 'charlie', 'eve'];
  ...
  it('should handle quoted vs unquoted template expressions correctly', () => {
    ...
    const quotedResult = evaluator.evaluate(
      '$.data | where($.balance == "{{param:maxBalance}}")',
      testData
    );
  ...
  it('should throw clear errors for missing parameters', () => {
    expect(() =>
      evaluator.evaluate('$.data | where($.balance > {{param:nonexistentParam}})', testData)
  ...
  it('should support response template expressions', () => {
    ...
    const result = evaluator.evaluate(
      '$.data | where($.balance > {{res:step1-0.$.threshold}})',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
  ...
  it('should support transformed data template expressions', () => {
    ...
    const result = evaluator.evaluate(
      '$.data | where($.balance >= {{proc:step1-0.$.filterConfig.minAmount}})',
      testData
    );
    const expected = [
      { username: 'alice', balance: 1500, status: 'active', category: 'premium' },
