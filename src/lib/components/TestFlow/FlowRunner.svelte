<script lang="ts">
  ...
  $: console.log("flow data", flowData);
	...
	function processTemplateExpression(expression: string, storedData: Record<string, any>, originalMatch: string): string {
	...
	function resolveTemplateValue(value: string, storedData: Record<string, any>): string {
		...
		value = value.replace(/"(\{\{\{([^}]+)\}\}\})"/g, (match, bracketExpr, expression) => {
			return processTemplateExpression(expression, storedData, match);
		});
    ...
    console.log("before processed: ", objString);
		const processedStr = resolveTemplateValue(objString, storedData);
    console.log("after processed: ", processedStr);
	...
	function resolveResponseTemplate(path: string, storedData: Record<string, any>): string {
...
<div class="mb-6 rounded-lg border bg-white p-4 shadow-sm">
	<div class="mb-4 flex items-center justify-between">
		...
		<div class="flex space-x-3">
			<button
				class="rounded-md px-4 py-2 text-sm font-medium {isRunning
				...
				{#if isRunning}
					<div class="flex items-center">
						<div class="mr-2 h-4 w-4">
							<div
								class="h-full w-full animate-spin rounded-full border-2 border-gray-600 border-t-transparent"
							></div>
						</div>
						Running...
					</div>
			...
			<button
				class="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium {isRunning
	...
	<!-- Error Display -->
	{#if error}
		<div class="mt-4 rounded-md border border-red-300 bg-red-100 p-3 text-red-800">
			<div class="font-medium">Flow execution error</div>
			<div class="mt-1 text-sm">{error.message || String(error)}</div>
		</div>
	{/if}

	<!-- Preferences Section -->
	<div class="mt-4 border-t pt-4">
		<h4 class="mb-3 text-sm font-medium text-gray-700">Execution Preferences</h4>
		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			...
			<div class="flex items-center">
				<input
					...
					disabled={isRunning}
				/>
				<label for="server-cookie-handling" class="ml-2 flex items-center text-sm text-gray-700">
					Use server for cookie handling
					<span class="ml-1 rounded bg-gray-100 px-1 text-xs text-gray-500"> CORS fix </span>
				</label>
			</div>

			<div>
				<label for="retry-count" class="block text-sm text-gray-700"> Retry attempts </label>
				<input
					type="number"
					id="retry-count"
					...
					min="0"
					max="5"
					class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm shadow-sm focus:border-blue-500 focus:outline-none"
					disabled={isRunning}
				/>
			</div>

			<div>
				<label for="timeout" class="block text-sm text-gray-700"> Timeout (ms) </label>
				<input
					type="number"
					id="timeout"
	...
	<div class="mt-6 border-t pt-4">
		...
		<div
			class="max-h-96 overflow-y-auto rounded-md bg-gray-800 p-2 font-mono text-xs text-gray-200"
		>
			{#if executionLogs.length === 0}
				<div class="p-2 text-gray-400 italic">No logs yet</div>
			{:else}
              ...
              "
							...
							<span class="text-xxs ml-2 text-gray-400">{log.timestamp.toLocaleTimeString()}</span>
						</div>
						{#if log.details}
							<div
								class="mt-1 ml-18 border-l-2 border-gray-600 pl-2 whitespace-pre-wrap text-gray-400"
							>
								{log.details}
							</div>
						{/if}
